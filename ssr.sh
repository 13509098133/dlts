...
#wget -N –no-check-certificate https://raw.githubusercontent.com/13509098133/dlts/master/ssr.sh && chmod +x ssr.sh && bash ssr.sh
...
Set_config_port(){
	...
	read -e $1 -p "(默认: 21314):" ssr_port
	[[ -z "$ssr_port" ]] && ssr_port="21314"
...
Set_config_password(){
	...
	read -e $1 -p "(默认: 21314.ssr):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="21314.ssr"
...
Set_config_method(){
 ...
 ${Tip} salsa20/chacha20-*系列加密方式，需要额外安装依赖 libsodium ，否则会无法启动ShadowsocksR !" && echo
	read -e $1 -p "(默认: 10. aes-256-cfb):" ssr_method
...
Set_config_protocol(){
 ...
 ${Tip} 如果使用 auth_chain_a 协议，请加密方式选择 none，混淆随意(建议 plain)" && echo
	read -e $1 -p "(默认: 2. auth_sha1_v4):" ssr_protocol
	...
	if [[ ${ssr_protocol} != "origin" ]]; then
		if [[ ${ssr_protocol} == "auth_sha1_v4" ]]; then
			read -e $1 -p "是否设置 协议插件兼容原版(_compatible)？[Y/n]" ssr_protocol_yn
...
Set_config_obfs(){
 ...
 如果你是在日本、美国等热门地区搭建，那么选择 plain 混淆可能被墙几率更低 !" && echo
	read -e $1 -p "(默认: 2. http_simple):" ssr_obfs
	...
	if [[ ${ssr_obfs} != "plain" ]]; then
			read -e $1 -p "是否设置 混淆插件兼容原版(_compatible)？[Y/n]" ssr_obfs_yn
...
Set_config_protocol_param(){
	...
	read -e $1 -p "(默认: 无限):" ssr_protocol_param
...
Set_config_speed_limit_per_con(){
	...
	read -e $1 -p "(默认: 无限):" ssr_speed_limit_per_con
...
Set_config_speed_limit_per_user(){
	...
	read -e $1 -p "(默认: 无限):" ssr_speed_limit_per_user
...
Set_config_all(){
	Set_config_port $1
	Set_config_password $1
	Set_config_method $1
	Set_config_protocol $1
	Set_config_obfs $1
	Set_config_protocol_param $1
	Set_config_speed_limit_per_con $1
	Set_config_speed_limit_per_user $1
}
# 修改 配置信息
Modify_config_port(){
...
echo -e "  ShadowsocksR 一键管理脚本 ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}
  ...
  ${Green_font_prefix}0.${Font_color_suffix} 全自动安装 ShadowsocksR
