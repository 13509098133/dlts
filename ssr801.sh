...
#wget -N –no-check-certificate https://raw.githubusercontent.com/13509098133/dlts/master/ssr801.sh
...
Set_config_port(){
	...
	read -e $1 -p "(默认: 3333):" ssr_port
	[[ -z "$ssr_port" ]] && ssr_port="3333"
...
Set_config_password(){
	...
	read -e $1 -p "(默认: 3333.ssr):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="3333.ssr"
...
Set_config_method(){
 ...
 ${Tip} salsa20/chacha20-*系列加密方式，需要额外安装依赖 libsodium ，否则会无法启动ShadowsocksR !" && echo
	read -e $1 -p "(默认: 5. aes-128-ctr):" ssr_method
...
Set_config_obfs(){
 ...
 如果你是在日本、美国等热门地区搭建，那么选择 plain 混淆可能被墙几率更低 !" && echo
	read -e $1 -p "(默认: 1. plain):" ssr_obfs
...
echo -e "  ShadowsocksR 一键管理脚本 ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}
  ...
  ${Green_font_prefix}0.${Font_color_suffix} centos8全自动安装 ShadowsocksR
